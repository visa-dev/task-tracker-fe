...
export default function SummaryCards({ stats, isAdmin, filters, onChange }) {
  ...
  return (
    <div
      className={`mb-6 grid grid-cols-2 gap-4 ${isAdmin ? 'sm:grid-cols-3 lg:grid-cols-6' : 'sm:grid-cols-5'}`}
    >
      {cards.map((c) => {
        const clickable = c.key === 'overdue' || c.key === 'unassigned'
        const active =
          (c.key === 'overdue' && !!filters.overdue) ||
          (c.key === 'unassigned' && !!filters.unassigned)
        ...
        return (
          ...
          >
            ...
            <div
              className={`mb-2 inline-flex h-8 w-8 items-center justify-center rounded-lg ${c.bg} ${c.text} text-xs font-bold`}
            >
              {values[c.key]}
            </div>
            ...
            {clickable && (
              <p className="mt-0.5 text-[11px] text-slate-400">
                {active ? 'Click to clear' : 'Click to view'}
              </p>
            )}
          </button>
        )
